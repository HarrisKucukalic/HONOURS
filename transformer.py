...
class TransformerModel(nn.Module):
    ...
    def __init__(self, region: str, input_shape: int, model_dim: int = 128, n_heads: int = 4, n_encoder_layers: int = 3,
    ...
    def train_model(self, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray,
                    epochs: int = 500, batch_size: int = 128, learning_rate: float = 1e-4, patience: int = 10):
        """
    ...
    def predict(self, X_test: np.ndarray, batch_size: int = 128) -> np.ndarray:
        """Makes predictions on new data in batches to conserve memory."""
