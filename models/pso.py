...
class PSO_Optimiser:
        ...
        for i in range(n_particles):
            ...
            if self.model_class == ANNModel:
                ...
                for j in range(num_layers):
                    # Get the exponent value using the CORRECT key name
                    power = int(np.round(params[f'neurons_power_layer_{j}']))  # Use 'neurons_power_layer'
                    # Calculate 2 to the power of that exponent
                ...
                n_heads = 2 ** int(np.round(params['n_heads']))
                d_model = int(np.round(params['model_dim'] / n_heads)) * n_heads
    ...
    def find_optimal_hyperparameters(self, search_space: dict, X_train, y_train, X_val, y_val, n_particles: int, iters: int) -> dict:
